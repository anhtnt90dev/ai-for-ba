...
description: "Nhận diện pattern hallucination và thiết kế prompt buộc câu trả lời bám vào nguồn dữ liệu."
---
...
- Giải thích hallucination và source grounding bằng ngôn ngữ business.
...
Nhận diện pattern hallucination và thiết kế prompt buộc câu trả lời bám vào nguồn dữ liệu.
</div>
...
Khi so sánh vendor, AI có thể tự tin bịa capability của sản phẩm. BA cần yêu cầu citation, tách evidence khỏi inference và đánh dấu claim thiếu nguồn là risk.
...
flowchart LR
    ...
    E --> F["Hallucination và source grounding"]
```
...
```text
Chỉ trả lời dựa trên nguồn được cung cấp. Với mỗi claim, ghi source ID. Nếu nguồn không hỗ trợ claim, ghi 'not supported by provided sources'.
```
