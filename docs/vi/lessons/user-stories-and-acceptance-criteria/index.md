...
description: "Dùng AI để draft user story và acceptance criteria nhưng vẫn giữ business intent, edge case và testability."
---
...
- Giải thích user story và acceptance criteria bằng ngôn ngữ business.
...
Dùng AI để draft user story và acceptance criteria nhưng vẫn giữ business intent, edge case và testability.
</div>
...
Một request mơ hồ nói: 'Users can update profiles.' BA dùng AI để tách actor goal, field được sửa, validation, permission, audit, error handling và acceptance criteria.
...
flowchart LR
    ...
    E --> F["User story và acceptance criteria"]
```
...
```text
Chuyển requirement này thành user story và acceptance criteria dạng Given-When-Then. Bao gồm negative case, permission, audit need và câu hỏi chưa rõ.
```
